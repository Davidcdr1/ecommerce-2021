...
export const ProductBabyes = () => {

  const [productBabyesList, setProductBabyesList] = useState([]);
  ...
  useEffect(() => {
    ...
    db.collection("productsbabyes").onSnapshot((results) => {
      const productsBabyes = [];
      results.forEach((productb) => {
        productsBabyes.push({ id: productb.id, ...productb.data() });
      });
      setProductBabyesList(productsBabyes);
    });
  }, []);
